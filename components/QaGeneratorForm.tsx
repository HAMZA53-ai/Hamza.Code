...
interface QaGeneratorFormProps {
    prompt: string;
...
export const QaGeneratorForm: React.FC<QaGeneratorFormProps> = ({
    prompt, setPrompt, isGenerating, onGenerate,
}) => {
    return (
        <div className="bg-gray-800 p-6 rounded-xl shadow-2xl space-y-6 sticky top-24">
            <h2 className="text-xl font-semibold text-white">نموذج حمزة كود</h2>
            
            <div className="border-t border-gray-700"></div>

            <div className="space-y-2">
                <label htmlFor="prompt" className="block font-medium text-gray-300">اطرح أي سؤال</label>
                <textarea
                    id="prompt"
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder="مثال: ما هي عاصمة أستراليا؟ أو اشرح لي كيفية عمل الشبكات العصبية."
                    rows={8}
                ...
                />
                 <p className="text-xs text-gray-500 text-center px-2 pt-1">
                    يمكنك طرح أسئلة عامة، أو طلب شرح لمفاهيم معقدة، أو المساعدة في البرمجة.
                </p>
            </div>
            ...
            >
                {isGenerating ? (
                    ...
                    <><IconHelpCircle className="w-6 h-6" /><span>اسأل النموذج</span></>
                )}
            </button>
        </div>
    );
